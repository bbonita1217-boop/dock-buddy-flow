...
function Containers() {
  ...
  return (
    <>
      <PageHeader
        title="컨테이너"
        description="ETA·통관·배차 상태를 관리하고 수동으로 일정을 조정합니다"
      />
                    ...
                    <tr key={c.id} className="border-t hover:bg-muted/30">
                      ...
                      <td className="px-2 py-1.5">
                        <select
                          ...
                          onChange={(e) =>
                            updateContainer(c.id, { warehouse_id: e.target.value || null })
                          }
                      ...
                      <td className="px-2 py-1.5">
                        <input
                          ...
                          onChange={(e) =>
                            updateContainer(c.id, { inbound_date: e.target.value || null })
                          }
                      ...
                      <td className="px-2 py-1.5">
                        <select
                          ...
                          onChange={(e) =>
                            updateContainer(c.id, { inbound_time: e.target.value || null })
                          }
                          className="h-7 text-xs rounded border bg-background px-1.5"
                        >
                          <option value="">-</option>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={13} className="text-center py-12 text-sm text-muted-foreground">
                      데이터가 없습니다. 엑셀 업로드 페이지에서 데이터를 등록해주세요.
                    </td>
                  </tr>
                )}
              </tbody>
